...
func Generate(fs http.FileSystem, opts Options) error {
	...
	walk := func(path string, info os.FileInfo, rs io.ReadSeeker, err error) error {
		...
		ident, ok := f.(Identity)
		if !ok {
			return fmt.Errorf("type conversion to Identity failed")
		}
...
var t = template.Must(template.New("").Funcs(template.FuncMap{
	"quote": strconv.Quote,
}).Parse(`{{define "Header"}}// Code generated by idemvfs; DO NOT EDIT.
...
func (f fileIdentifier) Identify(name string) (Identity, bool) {
	ident, ok := f[name]
	return ident, ok
}
